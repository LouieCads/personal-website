	...
	const actionCommands = [
		{
			...
			action: () => window.open('https://github.com/LouieCads', '_blank')
		},
		{
			cmd: '/linkedin',
		...
		},
		{
			...
			action: () => window.open('https://www.instagram.com/louie.21_/', '_blank')
		},
		{
			cmd: '/x',
			desc: 'open X profile',
			action: () => window.open('https://x.com/louigie_21', '_blank')
		},
		{
			cmd: '/email',
			desc: 'open email client',
			action: () => window.open('https://mail.google.com/mail/u/0/#all?compose=new', '_blank')
		}
	];
...
<div
	class="view-enter flex h-full flex-col gap-4 px-4 py-4 sm:gap-5 sm:px-8 sm:py-6 md:gap-6 md:px-16 md:py-8 lg:px-18"
>
		...
		<div class="grid gap-4 lg:grid-cols-3">
			...
			<div class="border border-(--color-border) bg-(--color-surface-card)">
				<div class="flex items-center gap-2 border-b border-(--color-border) px-4 py-3">
					<span class="font-mono text-[10px] tracking-widest text-(--color-text-muted)"
						>NAVIGATION</span
					>
					<span class="h-px flex-1 bg-(--color-border)"></span>
					<span class="font-mono text-[10px] text-(--color-text-muted)"
						>{staticCommands.length}</span
					>
				</div>
				<div class="divide-y divide-(--color-border)">
					{#each staticCommands as { cmd, desc } (cmd)}
						<div class="flex items-center gap-4 px-4 py-3">
							<code class="w-28 shrink-0 font-mono text-sm text-(--color-text-secondary)"
								>{cmd}</code
							>
				...
				<div class="divide-y divide-(--color-border)">
					{#each actionCommands as { cmd, desc, action } (cmd)}
						...
						>
							...
							<span
								class="flex-1 font-mono text-xs text-(--color-text-muted) transition-colors group-hover:text-(--color-text-secondary)"
								>{desc}</span
							>
							<span
								class="font-mono text-[10px] text-(--color-text-muted) opacity-0 transition-opacity group-hover:opacity-100"
								>&nearr;</span
							>
						</button>
					{/each}
